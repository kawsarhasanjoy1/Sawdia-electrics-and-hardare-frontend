...
const Category = () => {
  ...
  const { data, isFetching } = useGetAllCategoryQuery(query);
  const categories = data?.data?.data || [];

  const [softDelete] = useDeletedCategoryMutation();
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: any) => <span>{row.parentCategory?.name || "-"}</span>,
    },
    {
      key: "status",
      header: "Status",
  ...
  return (
    <div className="p-6 shadow-xl rounded-2xl bg-white">
      <div className="  mb-6 text-gray-800 flex justify-between items-center gap-6 md:gap-0 pt-6">
        <p className="text-2xl font-bold">{"Sub category "}</p>
        <SearchBox
      ...
      {isFetching ? (
        ...
        <NotFound message="No sub categories exist at this moment" />
      )}
